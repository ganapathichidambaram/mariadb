...
#        handle -lmariadb   -lpthread -lz -ldl -lm -lssl -lcrypto  -Wl,--no-undefined   and other strings inserted that might contain
...
my $pkglibdir = fix_path('/usr/lib64',"libmysql/relwithdebinfo",
                         "libmysql/release","libmysql/debug","lib/mysql","lib");

my $pkgincludedir = fix_path('/usr/include/mysql', "include/mysql", "include");
...
$flags->{libs} =
  [@ldflags,@lib_opts,'','','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'','-lmariadb   -lpthread -lz -ldl -lm -lssl -lcrypto  -Wl,--no-undefined  ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'dl','','-lmariadb   -lpthread -lz -ldl -lm -lssl -lcrypto  -Wl,--no-undefined  ','',''];
