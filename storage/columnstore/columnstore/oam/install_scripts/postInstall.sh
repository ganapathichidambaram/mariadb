...
if { $CONFIGFILE == " " } {
	set CONFIGFILE /etc/columnstore/Columnstore.xml.rpmsave
}
...
expect -re "# "
log_user 0
exec mv -f /etc/columnstore/Columnstore.xml /etc/columnstore/Columnstore.xml.new  > /dev/null 2>&1
exec mv -f $CONFIGFILE  /etc/columnstore/Columnstore.xml  > /dev/null 2>&1
